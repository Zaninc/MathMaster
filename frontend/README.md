...
- **Aprendizado**: sistema real de exercícios de múltipla escolha (Sprint V1.5.2) — tópicos e exercícios vêm das tabelas `topics`/`exercises` do Supabase (leitura só autenticada via RLS; deslogado vê convite de login), correção instantânea no cliente, nada persistido (histórico/estatísticas ficam para sprints futuras). O preview antigo da Learning Engine (`LearningDashboard`) foi removido; `data/learning-preview.ts` e `DomainMeter` continuam existindo porque a Home (`ProgressPreview`) ainda os usa.
