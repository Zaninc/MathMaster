 ...
 * Este componente cuida do que é específico de Geometria: desenhar o
 * `GeometryShape` já calculado a partir dos NÚMEROS que o usuário
 * digitou (nunca da resposta em texto do backend) — cálculos e desenhos
 * de triângulo/círculo/reta/parábola/elipse/hipérbole intocados. As
 * marcações (pontos, rótulos, segmentos auxiliares) vêm de
 * `GeometryMarkers.tsx`, separado de propósito da lógica matemática.
 */
export function GeometryCanvas({ shape, viewport, onViewportChange }: GeometryCanvasProps) {
  return (
    ...
    >
      {(context) => {
        const { viewport: renderViewport, width, height } = context;
        return (
          <>
            {shape?.kind === "triangle" && (
              <>
                <path
                  ...
                  strokeWidth={2}
                />
                <TriangleMarkers shape={shape} context={context} />
              </>
            )}
            ...
            {shape?.kind === "circle" && (
              <>
                <circle
                  ...
                  strokeWidth={2}
                />
                <CircleMarkers shape={shape} context={context} />
              </>
            )}
            ...
            {shape?.kind === "line" && (
              <>
                <line
                  ...
                  strokeWidth={2}
                />
                <LineMarkers shape={shape} context={context} />
              </>
            )}
            ...
            {shape?.kind === "parabola" && (
              <>
                <path
                  ...
                  strokeWidth={2}
                />
                <ParabolaMarkers shape={shape} context={context} />
              </>
            )}
            ...
            {shape?.kind === "ellipse" && (
              <>
                <path
                  ...
                  strokeWidth={2}
                />
                <EllipseMarkers shape={shape} context={context} />
              </>
            )}
            ...
            {shape?.kind === "hyperbola" && (
              <>
                ...
                <path
                  ...
                  strokeWidth={2}
                />
                <HyperbolaMarkers shape={shape} context={context} />
              </>
            )}
          </>
        );
